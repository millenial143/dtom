...
module.exports.run = async function(client, message, args, mysqlConnection, guildId){
	...
	if (message.mentions.roles.size == 0 && !message.mentions.everyone){
		message.mentions.users.forEach((value, key) => {
			let iddd = `${key}` + "";
			mysqlConnection.query(`SELECT id FROM bull${message.guild.id} WHERE bully_users = ${iddd}`
				, function(err, results, fields){	
					if (results.length == 0){
						mysqlConnection.query(`INSERT INTO bull${message.guild.id} (bully_users) VALUES('${key}')`
							, function(err, results, fields){
							});
					}else{
					}			
